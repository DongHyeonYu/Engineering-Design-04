...
float dist_min, dist_max, dist_raw, median, alpha; // unit: mm
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  count++;
  if (count == 1) first_dist = reading;
  else if (count == 2) second_dist= reading;
  else if (count == 3) third_dist = reading;
  return reading;
}
