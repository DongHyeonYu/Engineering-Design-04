...
float dist_min, dist_max, dist_raw, dist_prev, dist_ema, dist_LED; // unit: mm
float duty_per_degree;
...
// millis() returns thLe number of milliseconds since the program started. Will overflow after 50 days.
  if(millis() < last_sampling_time + INTERVAL) return;
  dist_LED = _ALPHA*dist_LED + (1-_ALPHA)*dist_LED;
  if ((180<=dist_LED) &&(dist_LED<=360)){
...
// get a distance reading from the USS
  ...
  dist_ema = _ALPHA*dist_raw  + (1-_ALPHA)*dist_ema;
...
// adjust servo position according to the USS read value
  ...
  if(dist_ema < 180.0) {
    ...
    duty_per_degree = _DUTY_MIN + 10.3 * (dist_ema-180); // (_DUTY_MAX-_DUTY_MIN)/180 = 10.255555...
    myservo.writeMicroseconds(duty_per_degree);
  }
